...
function ProveedorList({ proveedores = [], onEdit, onDelete }) {
  ...
  const getProveedorStatus = (proveedor) => {
    // Esta lógica puede adaptarse según los datos reales de tus proveedores
    const isActive = proveedor.activo !== undefined ? proveedor.activo : true;
            ...
            <table className="min-w-full divide-y divide-slate-200">
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {currentProveedores.map((proveedor) => {
                  const status = getProveedorStatus(proveedor);
                  
                  return (
                    <tr key={proveedor.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="h-8 w-8 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 text-xs font-medium mr-3">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm text-slate-600 capitalize">
                          {proveedor.forma_pago ? proveedor.forma_pago.toLowerCase() : '-'}
                        </div>
          ...
          {proveedores.length > itemsPerPage && (
            <div className="px-4 py-3 flex items-center justify-between border-t border-slate-200 sm:px-6">
              <div className="flex-1 flex justify-between sm:hidden">
                ...
                <button
                  onClick={() => setCurrentPage(currentPage < Math.ceil(proveedores.length / itemsPerPage) ? currentPage + 1 : currentPage)}
                  ...
                  className={`ml-3 relative inline-flex items-center px-4 py-2 border border-slate-300 text-sm font-medium rounded-md ${
                    currentPage >= Math.ceil(proveedores.length / itemsPerPage)
                      ? 'bg-slate-50 text-slate-400 cursor-not-allowed'
                      : 'bg-white text-slate-700 hover:bg-slate-50'
                  }`}
                >
              ...
              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-slate-700">
                    ...
                    <span className="font-medium">
                      {indexOfLastItem > proveedores.length ? proveedores.length : indexOfLastItem}
                    </span>{' '}
                ...
                <div>
                  <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                    ...
                    {/* Páginas */}
                    {Array.from({ length: Math.min(5, Math.ceil(proveedores.length / itemsPerPage)) }).map((_, idx) => {
