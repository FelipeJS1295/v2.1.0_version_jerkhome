...
function VentasImportarWalmart() {
  ...
  const eliminarVenta = (index) => {
    const nuevasVentas = [...ventasPreview];
    nuevasVentas.splice(index, 1);
    setVentasPreview(nuevasVentas);
  };

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">🛒 Importar Ventas - Walmart</h1>
            ...
            <table className="w-full border text-sm">
              ...
              <tbody>
                {ventasPreview.map((venta, index) => (
                  <tr
                    key={index}
                    className={`border-t ${venta.ya_existe ? 'bg-red-100' : ''}`}
                  >
...
export default VentasImportarWalmart;